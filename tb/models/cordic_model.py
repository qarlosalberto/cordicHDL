...
class Cordic :
    ...
    def arcMag(self,x,y):
        arrX = np.zeros(self.resolution+1)
        arrY = np.zeros(self.resolution+1)
        arrZ = np.zeros(self.resolution+1)
        arrP = np.zeros(self.resolution+1)
        arrA = np.zeros(self.resolution+1)
        ########################################################################
        arrX[0] = float(x)
        ...
        arrP[0] = 1.0
        ########################################################################
        for i in range(1,len(self.angulos)) :
            if arrY[i-1]>=0.0 :
                arrX[i] = arrX[i-1] + arrY[i-1]/arrP[i-1]
                arrY[i] = arrY[i-1] - arrX[i-1]/arrP[i-1]
                arrZ[i] = arrZ[i-1] + self.angulos[i-1]
            else :
                arrX[i] = arrX[i-1] - arrY[i-1]/arrP[i-1]
                arrY[i] = arrY[i-1] + arrX[i-1]/arrP[i-1]
                arrZ[i] = arrZ[i-1] - self.angulos[i-1]
            arrP[i]= arrP[i-1]*2.0
        ########################################################################
        arctg = arrZ[self.resolution-1]
        mag   = arrX[self.resolution-1]*self.c
        return arctg,mag
    ...
    def testInput(self):
        ...
        while(True):
            ...
            sin, cos = c.sincos(x)
            arc, mag = c.arcMag(x,y)

            print( "Seno: " + str(sin) + " Coseno: " + str(cos))
            print( "Arctg: " + str(arc) + " Magnitude: " + str(mag))
